...
import com.custommapsapp.android.kml.*;
...
public class CustomMapFragment extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        Log.i(LOG_TAG, "App memory available (MB): " + MemoryUtil.getTotalAppMemoryMB(getActivity()));
        boolean ptSizeFixNeeded = PtSizeFixer.isFixNeeded(getActivity());
        if (ptSizeFixNeeded) {
            PtSizeFixer.fixView(detailsDisplay.getRootView());
        }

        // Initialize disk cache properly
        ImageDiskCache.instance(getActivity());

        GeoidHeightEstimator.initialize(getActivity().getAssets());

        // Do not process launch intent if software license is not accepted yet
        if (locator == null) {
            locator = (LocationManager) getActivity().getSystemService(Activity.LOCATION_SERVICE);
        }
        if (sensors == null) {
            sensors = (SensorManager) getActivity().getSystemService(Activity.SENSOR_SERVICE);
        }
        PreferenceStore.instance(getActivity()).setLicenseAccepted(true);
        if (PreferenceStore.instance(getActivity()).isLicenseAccepted()) {
    ...
    public void onInflate(Activity activity, AttributeSet attrs, Bundle savedInstanceState) {
        ...
        TypedArray a = activity.obtainStyledAttributes(attrs, R.styleable.CustomMapFragment);
        CharSequence myString = a.getText(R.styleable.CustomMapFragment_map_file);
        this.mapFileName = myString.toString();
        a.recycle();
    }
...
//        setContentView(R.layout.main);
        mapDisplay = (MapDisplay) getActivity().findViewById(R.id.mapDisplay);
        if (inertiaScroller == null) {
            inertiaScroller = new InertiaScroller(mapDisplay);
        } else {
            inertiaScroller.setMap(mapDisplay);
        }
        locationLayer = (LocationLayer) getActivity().findViewById(R.id.locationLayer);
        displayState = new DisplayState();
        mapDisplay.setDisplayState(displayState);
        locationLayer.setDisplayState(displayState);
        distanceLayer = (DistanceLayer) getActivity().findViewById(R.id.distanceLayer);
        distanceLayer.setDisplayState(displayState);
        detailsDisplay = (DetailsDisplay) getActivity().findViewById(R.id.detailsDisplay);
        ...
        zoomIn = (ImageButton) getActivity().findViewById(R.id.zoomIn);
        zoomOut = (ImageButton) getActivity().findViewById(R.id.zoomOut);
    ...
    private KmlFolder loadFileWhichIChoose() {
        try {
            Iterable<KmlInfo> kmlFiles = KmlFinder.findEuropeMap(FileUtil.getDataDirectory(),mapFileName);
    ...
    public void onResume() {
        ...
        if (!PreferenceStore.instance(getActivity()).isLicenseAccepted() && !licenseDialogCreated) {
            licenseDialogCreated = true;
            Log.i(LOG_TAG, "Showing license dialog");
            getActivity().showDialog(DIALOG_LICENSE);
            return;
        }

        if (mapDisplay.getMap() == null && getActivity().getIntent().getBundleExtra(SAVED_INSTANCESTATE) != null) {
            selectedMap = null;
            onActivityCreated(getActivity().getIntent().getBundleExtra(SAVED_INSTANCESTATE));
        }
        locationTracker.setContext(getActivity().getApplicationContext());
        locationTracker.setDisplay(getActivity().getWindowManager().getDefaultDisplay());
        ...
        sensors.registerListener(locationTracker, sensor, 50000);
        PreferenceStore prefs = PreferenceStore.instance(getActivity().getApplicationContext());
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        getActivity().getIntent().putExtra(SAVED_INSTANCESTATE, outState);
    }
    ...
    public void onActivityCreated(Bundle inState) {
        ...
        if (selectedMap != null || inState == null) {
        ...
        getActivity().getIntent().removeExtra(SAVED_INSTANCESTATE);
    }
...
//    }
    ...
    private void displayUserMessage(final String message) {
        ...
        getActivity().runOnUiThread(displayTask);
    }
...
//    }
    ...
    private void toggleDetailsDisplay() {
        ...
        PreferenceStore.instance(getActivity().getApplicationContext()).setShowDetails(showDetails);
    }

    private void shareMap() {
        if (selectedMap == null || !FileUtil.shareMap(getActivity(), selectedMap)) {
    ...
    private boolean launchIntentAction(String action, Uri data) {
        ...
        if (ContentResolver.SCHEME_CONTENT.equals(data.getScheme())) {
            File savedFile = FileUtil.saveKmzContentUri(getActivity(), data);
            if (savedFile != null) {
    ...
    private void launchKmzDownloader(URL mapUrl) {
        Intent downloadMap = new Intent(getActivity(), KmzDownloader.class);
    ...
    private void launchEditPreferences() {
        Intent editPrefs = new Intent(getActivity(), EditPreferences.class);
        startActivityForResult(editPrefs, EDIT_PREFS);
    }

    private void launchSelectMap(File localFile) {
        Intent selectMap = new Intent(getActivity(), SelectMap.class);
    ...
    private void launchSelectMap(Location location) {
        ...
        selectMap.putExtra(SelectMap.AUTO_SELECT, true);
        processSelectMapResult(selectMap);
//        startActivityForResult(selectMap, SELECT_MAP);
    }
    ...
    private void processSelectMapResult(Intent data) {
        if (data == null) {
            ...
            Bundle instanceState = getActivity().getIntent().getBundleExtra(SAVED_INSTANCESTATE);
            if (instanceState != null && instanceState.containsKey(SAVED_MAP)) {
                selectedMap = null;
                onActivityCreated(instanceState);
                if (selectedMap == null) {
            ...
            initializeMapVariables(loadFileWhichIChoose());
            MapError status = loadMapForDisplay(mapImage, mapDisplay.getMap());
            if (status == MapError.NO_ERROR) {
                mapDisplay.addMapMarkers(placemarks);
                if (PreferenceStore.instance(getActivity()).isReminderRequested()) {
    ...
    private MapError loadMapForDisplay(GroundOverlay newMap, GroundOverlay oldMap) {
        ...
        } catch (MapDisplay.MapImageTooLargeException ex) {
    ...
    private void displaySafetyReminder(Bundle savedState) {
        safetyReminder = new SafetyWarningDialog(getActivity());
        safetyReminder.setOnDismissListener(new DialogInterface.OnDismissListener() {
            ...
            public void onDismiss(DialogInterface dialog) {
                ...
                if (reminder.wasButtonPressed() && !reminder.getShowAgainRequested()) {
                    PreferenceStore.instance(getActivity()).setReminderRequested(false);
                }
                safetyReminder = null;
            }
        });
...
//        if (id == DIALOG_LICENSE) {
//            String version = PreferenceStore.instance(getActivity()).getVersion();
